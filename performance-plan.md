...
### 実測（2026-09-10, 同一セッションで 3 モードを切り替え）

| モード | 結果 |
|---|---|
| 0 SNAPSHOT | 遅延とゴースト |
| 1 CLONE | 遅延とゴーストは消え、**パネルの 10px ずれ** |
| 2 SKIP | 遅延とゴーストが消え、パネルのクローンが消える |

**推論どおり。3 つの症状がすべて経路に紐づくことが確定した。**
とくに**テクスチャ遅延の再現条件がついに特定できた** ——
これまで間欠的に見えていたのは、BMS の有効化順序という隠れた変数のせいで
SNAPSHOT 経路に入ったり入らなかったりしていたため。

### 既定は SKIP にした

**破壊しないのは SKIP だけ**なので既定にした。

失うものはこの構成では小さい。BMS の dynamic panel blur が有効なら
**パネルの見た目はすでに「ぼかした壁紙」**であり、
壁紙自体はガラスに入っている。実際に失われるのはパネルの**中身**（時計・アイコン）で、
それは memo.md 追記3 の
「ガラスの向こうに見えるべきものはパネルの素材であって中身ではない」
という結論からすると、そもそも入れるべきではないもの。

デバッグ用に `global._lgGlass.bmsMode(n)` は残す。

### SNAPSHOT が遅延を生む機構（推定）

`SelfExcludingSnapshotCapture` は `after-paint` のたびに
**自分のガラスルートを hide → ステージ全体を paint_to_content → show** を行う。
memo.md 追記3 失敗3 が
「ホスト側の実 UI の可視性を毎フレーム切り替えると
map / relayout / damage / 入力の帳簿がすべて壊れる」
と記録しているのと同じことを、我々自身のルートに対してやっている。
ステージのダメージ帳簿が壊れれば、
「内容が 1 フレーム古い」という症状は自然に出る。

### 本命の解（未実装）

memo.md 追記3 が Quick Settings で同じ問題に到達し、
**`UnpickableStyledWidget`（実パネルの style class をコピーして
St に同じ背景を描かせる）** で決着している。
クローンもスナップショットも使わず、実パネルに一切触れない。

トップパネルにも同じ手が使えるはず。
「ガラスの向こうに見えるべきものはパネルの**素材**であって中身ではない」
という追記3 の結論はここでもそのまま当てはまる。
