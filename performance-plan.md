...
## 調査中: dock を「上」に置くと dock のガラスに dock 自身のアイコンが映る

### 最初の仮説は外れた

「`dockRoot` の除外が構築時のスナップショットで、Dash to Dock がコンテナを
作り直すと古くなる」と考えて `ancestorExclusions`（refresh のたびに
uiGroup 祖先を解決し直す）を実装したが、**外れ**。

追加した診断ログが決定的:

```
[Liquid Glass][ui-sampler:dock] cloning [panelBox, Meta_WindowGroup,
  modalDialogGroup, keyboardBox, screenshotUIGroup, Dummy Cursor,
  Meta_WindowGroup, Clutter_Actor]
```

**`dashtodockContainer` は入っていない** —— 除外は正しく効いている。
（比較: `[ui-sampler:osd] cloning [panelBox, dashtodockContainer, ...]`。
OSD が dock をクローンするのは正しい。）

`ancestorExclusions` 自体は「固定アクターのスナップショットより堅い」という
点で改善なので残す。

### 次の仮説: BMS スナップショット経路

`UILayerSampler._createSelfExcludingSnapshotActor()` は
Blur My Shell のターゲット（通常 `panelBox`）を クローン ではなく
`SelfExcludingSnapshotCapture` —— **ステージ全体をそのターゲットの矩形で
撮ったスナップショット** —— として供給する。
自己写り込みを避けるために隠すのは**自分自身のガラスルートだけ**で、
dock は隠さない。

したがって **dock がそのターゲットの矩形と重なると、
スナップショットに dock のアイコンが写り込む**。

- dock が下 → パネル矩形と重ならない → ゴーストなし
- dock が**上** → パネルと重なる → **ゴーストが出る**

報告された「上のときだけ」という条件を説明できる。
ユーザーはこのセッションで BMS を有効化している。

**確認済み（2026-09-09）**: **BMS を無効にするとゴーストは消えた。仮説は確定。**

**注意**: 修正としてスナップショット中に dock を隠すのは
**やってはいけない**（memo.md 追記3 失敗3:
ホスト側の実 UI の可視性を毎フレーム切り替えると
map / relayout / damage / 入力の帳簿がすべて壊れる）。
別の手を考える必要がある。
