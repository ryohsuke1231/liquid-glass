...
function _debugCheckStageSignals(): void {
  if (_signalExistenceChecked) return;
  _signalExistenceChecked = true;
  try {
    const gtype = (Clutter.Stage as any).$gtype;
    for (const name of ['paint-view', 'after-paint', 'presented', 'paint']) {
      try {
        const id = (GObject as any).signal_lookup(name, gtype);
 ...
 * 合わせて確認する。
 */
function _debugTryAfterPaintSignalCapture(stage: Clutter.Stage): void {
  if (_afterPaintExperimentDone) return;
  _afterPaintExperimentDone = true;
  try {
    const handlerId = (stage as any).connect('after-paint',
      (_stage: Clutter.Stage, view: Clutter.StageView, _frame: any) => {
        try {
          console.log(`[DelayedScreenCapture][DIAG][AFTER-PAINT-EXP] after-paint 発火 view=${view ? view.toString() : 'null'}`);
          const fb: any = (view as any)?.get_onscreen?.() ?? view?.get_framebuffer?.();
          if (!fb) {
          ...
          let nonZeroCount = 0;
          for (let i = 0; i < data.length; i++) { if (data[i] !== 0) nonZeroCount++; }
          console.log(
            `[DelayedScreenCapture][DIAG][AFTER-PAINT-EXP] 同期blit(${testX},${testY},${testW}x${testH}) ` +
          ...
          try { (stage as any).disconnect(handlerId); } catch (_) { /* noop */ }
        }
      });
  } catch (e) {
...
export class DelayedScreenCapture {
  private _tex: Cogl.Texture2D | null = null;
  private _destFb: Cogl.Offscreen | null = null; // blit経路用に使い回すFBO
  private _rectGetter: () => [number, number, number, number];
  private _afterPaintId: number = 0;
  private _debugRedrawClipLogCount: number = 0;
  private _stage: Clutter.Stage;
  private _refCount: number = 0;
  private _loggedOnce: boolean = false; // ← 診断ログを毎フレーム出さないためのフラグ
  private _loggedCycle: number = 0;

  constructor(stage: Clutter.Stage, rectGetter: () => [number, number, number, number]) {
    this._stage = stage;
    ...
    try {
      ...
      this._afterPaintId = this._stage.connect_after('paint-view', (_stage: Clutter.Stage, view: Clutter.StageView, redrawClip: any) => {
        ...
        try {
          ...
          // むしろ当然、ということになる。
          if (this._debugRedrawClipLogCount === undefined) this._debugRedrawClipLogCount = 0;
          if (this._debugRedrawClipLogCount < 8) {
            this._debugRedrawClipLogCount++;
            try {
              const panelTestRect = { x: 0, y: 0, width: 1920, height: 32 };
              let containsPanel: any = 'n/a';
              let extents: any = 'n/a';
              let isEmpty: any = 'n/a';
              try { isEmpty = typeof redrawClip?.is_empty === 'function' ? redrawClip.is_empty() : 'n/a'; } catch (_) { }
              try { extents = typeof redrawClip?.get_extents === 'function' ? JSON.stringify(redrawClip.get_extents()) : 'n/a'; } catch (_) { }
              try {
              ...
              console.log(
                `[DelayedScreenCapture][DIAG][REDRAW-CLIP] redrawClip=${redrawClip ? redrawClip.toString() : 'null'} ` +
  ...
  private _capture(view: Clutter.StageView): void {
    ...
    // 実体差を診断ログで確認する。
    let onscreenFb: any = null;
    let usedGetOnscreen = false;
    try {
      if (typeof (view as any).get_onscreen === 'function') {
        onscreenFb = (view as any).get_onscreen();
    ...
    if (!this._loggedOnce) {
      try {
        ...
        let shadowfb: any = 'n/a';
        try { shadowfb = typeof (view as any).has_shadowfb === 'function' ? (view as any).has_shadowfb() : 'n/a'; } catch (_) { }
        console.log(
          `[DelayedScreenCapture][DIAG][GET-ONSCREEN] get_onscreen()の有無=${typeof (view as any).get_onscreen === 'function'} ` +
          ...
          `get_onscreen()はCogl.Onscreenか=${onscreenFb ? (Cogl as any).Onscreen && onscreenFb instanceof (Cogl as any).Onscreen : 'n/a'}`
        );
      } catch (e) {
    ...
    try {
      ...
      if (!this._loggedOnce) {
        try {
          ...
          try { (ctx as any).flush?.(); } catch (_) { }
          try { (onscreenFb as any).finish?.(); } catch (_) { }
          ...
          for (let i = 0; i < data.length; i++) { if (data[i] !== 0) nonZeroCount++; }
          console.log(
            `[DelayedScreenCapture][DIAG][SANITY-BLIT] blit(${testX},${testY},${testW}x${testH}) ` +
            `returnedSize=${returnedSize} nonZeroBytes=${nonZeroCount}/${data.length} ` +
            `sample(10,10)=(${data[10 * rowStride + 10 * 4]},${data[10 * rowStride + 10 * 4 + 1]},${data[10 * rowStride + 10 * 4 + 2]},${data[10 * rowStride + 10 * 4 + 3]}) ` +
          ...
          try {
            ...
            let nz2 = 0;
            for (let i = 0; i < d2.length; i++) { if (d2[i] !== 0) nz2++; }
      ...
      if (!this._loggedOnce) {
        ...
        console.log(
          `[DelayedScreenCapture][DIAG] blit経路(usedGetOnscreen=${usedGetOnscreen}): src=(${srcX},${srcY}) size=${blitW}x${blitH}`
        );
        try {
          ...
          const dump = (label: string, px: number, py: number) => {
            const off = py * rowStride + px * 4;
            console.log(
              `[DelayedScreenCapture][DIAG][PIXEL] ${label}=(${px},${py}) ` +
  ...
  getTexture(): Cogl.Texture2D | null {
    return this._tex;
  }

  destroy(): void {
    if (this._afterPaintId) {
      try { this._stage.disconnect(this._afterPaintId); } catch (_) { }
...
export const PaintProbeEffect = GObject.registerClass(
  class PaintProbeEffect extends Clutter.Effect {
    ...
    declare private _windowStartUs: number;
    declare private _windowSamples: Array<{ offsetUs: number; fbW: number; fbH: number }>;
    declare private _totalCalls: number;

    _init(params?: any) {
      super._init(params);
      if (params && params.label) this._label = params.label;
    ...
    vfunc_paint(_node: Clutter.PaintNode, paintContext: Clutter.PaintContext): void {
      ...
      if (this._windowStartUs === 0 ||
        ...
        if (this._windowSamples.length > 1) {
          const detail = this._windowSamples
            ...
            .join(', ');
          console.log(
            `[Liquid Glass][DEBUG-BMS-PROBE][${this._label}] ` +
      ...
      const actor = this.get_actor() as any;
...
export type PaintProbeEffect = InstanceType<typeof PaintProbeEffect>;
...
export const PassthroughCaptureEffect = GObject.registerClass({
  ...
  // 意図的に空。デフォルトの OffscreenEffect 動作
  // (アクターの内容を FBO にキャプチャし、そのまま画面に描画する) だけを使う。
});
export type PassthroughCaptureEffect = InstanceType<typeof PassthroughCaptureEffect>;
...
export const TextureBlitActor = GObject.registerClass({
  ...
  declare private _flipY: boolean; // ← 追加
  declare private _opaqueBlend: boolean;
...
*/
  setOpaqueBlend(opaque: boolean): void {
...
const _delayedCaptureRegistry: Map<Clutter.Actor, DelayedScreenCapture> = new Map();
 ...
 */
function acquireDelayedCapture(
  sourceActor: Clutter.Actor,
  rectGetter: () => [number, number, number, number]
): DelayedScreenCapture {
  let capture = _delayedCaptureRegistry.get(sourceActor);
  if (!capture) {
    capture = new DelayedScreenCapture(global.stage as Clutter.Stage, rectGetter);
...
function releaseDelayedCapture(sourceActor: Clutter.Actor): void {
...
export class UILayerSampler {
  ...
  private _delayedCaptureOwners: Map<Clutter.Actor, Clutter.Actor> = new Map(); // clone-key -> sourceActor
   ...
   */
  private _createDelayedCaptureBlitActor(child: Clutter.Actor): Clutter.Actor | null {
    try {
    ...
    while (stack.length > 0) {
      ...
      try {
        ...
        for (const effect of effects) {
          ...
          if (gtypeName.startsWith('LiquidGlass')) continue; // 自前のデバッグ用は除外
  ...
  private _createExistingEffectBlitActor(child: Clutter.Actor): Clutter.Actor | null {
    ...
    if (found === undefined) {
      ...
      if (found) {
        const gtypeName = (found.effect.constructor as any)?.$gtype?.name ?? '(unknown)';
        console.log(
          `[Liquid Glass] 既存の OffscreenEffect を発見 (非侵襲で読み取り専用利用): ` +
          `actor=${found.actor.name || found.actor}, effectType=${gtypeName}`
        );
      } else {
        console.warn(
          `[Liquid Glass] ${child.name || child} 以下に既存の OffscreenEffect が` +
  ...
  refresh() {
    ...
    for (const child of children) {
      ...
      if (!this._clones.has(child)) {
        ...
        //    プローブが積み重なるのを防ぐため。
        if (this._debugBmsProbeEnabled && bmsTarget && !(bmsTarget as any)._liquidGlassProbeAttached) {
          try {
            ...
            (bmsTarget as any)._liquidGlassProbeAttached = true;
            this._debugProbedActors.set(bmsTarget, probe);
            console.log(
              '[Liquid Glass][DEBUG-BMS] BMS対象アクターにペイントプローブを取り付けました: ' +
              `${bmsTarget}`
            );
          } catch (e) {
  ...
  destroy() {
    ...
    for (const [actor, effect] of this._debugProbedActors) {
      try {
        if (!(actor as any)._isDisposed) {
          actor.remove_effect(effect);
          (actor as any)._liquidGlassProbeAttached = false;
        }
      } catch (_) { /* すでに破棄済みなら無視 */ }
    }
