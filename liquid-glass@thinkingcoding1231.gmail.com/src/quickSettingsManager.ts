...
export class QuickSettingsManager {
  ...
  private _enableSubmenuFix: boolean = false; // サブメニューを強制的にtranslation_x, translation_yで移動するかどうかのフラグ
  ...
  */
  ...
  _adjustSubmenuPositions() {
    ...
    if (Number.isNaN(parentAbsX) || Number.isNaN(parentAbsY) || Number.isNaN(parentW) || Number.isNaN(parentH) || parentW <= 0 || parentH <= 0) return;
    ...
    for (let submenu of foundMenus) {
      ...
      if (Number.isNaN(subAbsX) || Number.isNaN(subAbsY) || Number.isNaN(subW) || Number.isNaN(subH) || subW <= 0 || subH <= 0) continue;
      ...
      let findBoundaries = (n: Clutter.Actor) => {
        ...
        if (Number.isNaN(nodeY) || Number.isNaN(nodeW) || Number.isNaN(nodeH) || nodeH <= 5 || nodeW <= 5) return;
