...
export class QuickSettingsManager {
  ...
  private _toggleTintStrength: number = 0.5;
  ...
  _findAllToggleContainers(actor: Clutter.Actor, found: Clutter.Actor[] = []): Clutter.Actor[] {
    ...
    for (let child of children) this._findAllToggleContainers(child, found);
    return found;
  }
  ...
  _samplePodColor(pod: Clutter.Actor, primary: Clutter.Actor): { r: number; g: number; b: number; a: number } {
    ...
    if (isHasMenu && pod !== primary) {
      ...
      if (wrapperBg && wrapperBg.a > primaryBg.a) return wrapperBg;
    }
    return primaryBg;
  }
  ...
  private _panelSnapshotCapture: InstanceType<typeof SelfExcludingSnapshotCapture> | null = null;
  ...
  _ensurePanelContentClone(monitorX: number, monitorY: number): void {
    ...
    if (!this._panelSnapshotCapture && this.bgActor) {
      this._panelSnapshotCapture = new SelfExcludingSnapshotCapture(
        global.stage as Clutter.Stage,
        this.bgActor,
        () => {
          if (!this._menuRoot) return [0, 0, 0, 0];
    ...
    if (!this._panelContentClone || !isActorValid(this._panelContentClone) ||
      ...
      blit.setTextureGetter(() => {
        try {
          const content: any = this._panelSnapshotCapture?.getContent();
  ...
  _destroyPanelContentClone(): void {
    ...
    if (this._panelSnapshotCapture) {
      try { this._panelSnapshotCapture.destroy(); } catch (e) { }
  ...
  _syncToggleRegions() {
    ...
    let regions: { x: number; y: number; w: number; h: number; tintR: number; tintG: number; tintB: number }[] = [];
