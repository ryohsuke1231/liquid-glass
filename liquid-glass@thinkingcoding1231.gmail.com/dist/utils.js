...
export function addFrameTicker(cb, minIntervalMs = 0) {
    ...
    const schedule = () => {
        st.laterId = global.compositor.get_laters().add(Meta.LaterType.BEFORE_REDRAW, () => {
            ...
            if (keep && _frameTickers.get(id) === st)
                schedule();
            else if (_frameTickers.get(id) === st)
...
export function removeFrameTicker(id) {
    ...
    if (st.laterId) {
        try {
            global.compositor.get_laters().remove(st.laterId);
        }
        catch (_) { /* noop */ }
        st.laterId = 0;
    }
...
export class UILayerSampler {
    ...
    syncProperties(source, sourceClone, containerW, containerH, cX, cY) {
        ...
        try {
            ...
            const replicaForCull = sourceClone._lgBmsReplica;
